...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white sticky top-0 z-50 border-b border-(--my-green)">
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ${isOpen ? "max-h-96 border-t border-gray-100" : "max-h-0"}`}
      >
          ...
          {navItems.map((item, index) => (
            <li key={index}>
              <NavLink
                ...
                className={({ isActive }) =>
                  `block py-2.5 px-3 rounded-md font-semibold text-sm transition-colors duration-200 ${
                    isActive
